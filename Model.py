...
class Signaller(Agent):
    ...
    def __init__(self, player_type=1, signals=[0, 1, 2], responses=[0, 1]):
        ...
        self.response_belief = dict([(signal, dict([(response, []) for response in responses])) for signal in signals])
        self.type_distribution = dict([(signal, []) for signal in signals])
        self.type_matches = dict([(signal, 0.) for signal in signals])
        self.response_signal_matches = dict([(signal, dict([(response, 0.) for response in responses])) for signal in signals])

        super(Signaller, self).__init__(player_type, signals, responses)

    def init_payoffs(self, baby_payoffs, social_payoffs, type_weights=[1., 1., 1.], response_weights=[[1., 1.], [1., 1.], [1., 2.]]):
        # Don't set up twice.
    ...
    def update_beliefs(self, response, midwife_type, payoff):
